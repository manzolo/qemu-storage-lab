...
This protects data from a single disk failure." || return 0
    ...
    # Step 3: Verify RAID
    s=$(_next_step)
    tutor_step "$s" "Verify RAID array" \
        "cat /proc/mdstat" \
...
The PV will be /dev/md0, not /dev/vdb or /dev/vdc."
    ...
    s=$(_next_step)
    tutor_step "$s" "Create Physical Volume on RAID" \
...
LVM will see the RAID as a single physical disk." || return 0
    ...
    # Step 5: Verify PV
    s=$(_next_step)
    tutor_step "$s" "Verify Physical Volume" \
        "sudo pvs" \
...
All the RAID array's space is now in the LVM pool." || return 0
    ...
    tutor_step "$s" "Create a 2GB Logical Volume" \
        "sudo lvcreate -L 2G -n secure_lv raid_vg" \
...
disk -> RAID mirror -> PV -> VG -> LV -> ext4 -> file" || return 0
    ...
    s=$(_next_step)
    tutor_step "$s" "Full architecture overview" \
...
filesystem -> LVM (LV, VG, PV) -> RAID -> disks"
    ...
    # Step 14: Umount
    s=$(_next_step)
    tutor_step "$s" "Unmount filesystem" \
